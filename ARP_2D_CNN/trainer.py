...
class ARP2DCNNTrainer(object):

    def __init__(self, model_name, device='cpu', batch_size=16, init_lr=0.0001):
        setup_seed(1124)
        ...
        self.train_set = MRI2DDataset(train_set)
        ...
        self.optim = Adam(self.net.parameters(), lr=self.init_lr, weight_decay=0.001)
    ...
    def loss(pred, label, adas):
        return ARP2DCNNTrainer.cross_entropy_loss(pred, label)

    def train_an_epoch(self):
        self.net.train(True)
        for img, label, adas, benefit in self.train_dataloader:
            self.optim.zero_grad()
            img = img.to(self.device)
            label = label.to(self.device)
            adas = adas.to(self.device)
            pred = self.net(img)
            loss = self.loss(pred, label, adas)
            loss.backward()
            self.optim.step()
    ...
    def predict(self, stage):
        ...
        self.net.eval()
        with torch.no_grad():
            for i, (img, label, adas, benefit) in enumerate(data_loader):
                img = img.to(self.device)
                label = label.to(self.device)
                adas = adas.to(self.device)
                pred = self.net(img)
