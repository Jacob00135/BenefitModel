...
from models import DSA_3D_CNN_Zoom
...
class DSA3DCNNTrainer(object):

    def __init__(self, model_name, device='cpu', batch_size=16, init_lr=0.000015):
        setup_seed(1124)
        ...
        self.train_set = MRI3DDataset(train_set, mri_dir_path=zoom_mri_path)
        self.test_set = MRI3DDataset(test_set, mri_dir_path=zoom_mri_path)
        ...
        # 载入模型
        self.net = DSA_3D_CNN_Zoom().to(self.device)
        self.optim = Adam(self.net.parameters(), lr=self.init_lr, weight_decay=0.0)
        self.lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(self.optim, gamma=0.985)
        print('载入模型成功')
    ...
    def loss(pred, label, record):
        return DSA3DCNNTrainer.cross_entropy_loss(pred, label)
    ...
    def predict(self, stage):
        ...
        dataset = MRI3DDataset(df, mri_dir_path=zoom_mri_path)
