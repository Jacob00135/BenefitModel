...
class TransMFTrainerFRPLoss(TransMFTrainer):

    def __init__(self, adas_alpha, *args, **kwargs):
        super(TransMFTrainerFRPLoss, self).__init__(*args, **kwargs)
        self.adas_alpha = adas_alpha

    def frp_loss(self, label, pred, adas):
        delta = 1e-10
    ...
    def loss(self, label, adas, pred, mri_pred, pet_pred):
        ...
        loss_1 = self.frp_loss(label, pred, adas)
        loss_2 = self.frp_loss(mri_gt, mri_pred, adas)
        loss_3 = self.frp_loss(pet_gt, pet_pred, adas)
